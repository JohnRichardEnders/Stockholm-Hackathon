...
YouTubeFactChecker.prototype.addMorphStyles = function () {
  ...
  style.textContent = `
    ...
    .liquidGlass-effect { position: absolute; inset: 0; border-radius: inherit; filter: url(#glass-distortion); background: linear-gradient(135deg, rgba(142,197,255,0.35), rgba(180,144,255,0.35), rgba(255,209,255,0.35)); background-size: 200% 200%; animation: lg-shift 8s ease-in-out infinite; }
    .liquidGlass-tint { position: absolute; inset: 0; border-radius: inherit; background: rgba(10, 10, 20, 0.35); }
    .liquidGlass-shine { position: absolute; inset: 0; border-radius: inherit; background: radial-gradient(120% 120% at 100% 0%, rgba(255,255,255,0.35) 0%, rgba(255,255,255,0.05) 45%, rgba(255,255,255,0) 60%); filter: blur(4px); }
    .liquidGlass-text { position: relative; z-index: 1; display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; }
    @keyframes lg-shift { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }
...
YouTubeFactChecker.prototype.morphToCard = function (factCheckData = null) {
  ...
  requestAnimationFrame(() => {
    ...
    setTimeout(() => { this.applyBackgroundBlur(true); }, 60);
  });
...
YouTubeFactChecker.prototype.applyBackgroundBlur = function (shouldBlur) {
  ...
  if (shouldBlur) {
    ...
    setTimeout(() => { video.classList.remove('video-background-blur'); }, 300);
  }
};
...
YouTubeFactChecker.prototype.injectCardContent = function (factCheckData, keepHidden = false) {
  ...
  content.style.cssText = `
    ...
    color: white; width: 100%; padding-right: 48px; position: relative; z-index: 2; pointer-events: auto; box-sizing: border-box;
